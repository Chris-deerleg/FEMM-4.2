...
-- functions to create a particle to move through fields
  local t = {par={},Q=-1.6021766208e-19,M=9.10938356e-31,Els=ElecticalSim,Mas=MagneticalSim}--E[V/m]; x[mm]; s= stepsize 1mm;q=charge of electron; m=mass of a electron 
	...
	if xa2~=xa1 then 
		...
		t.par[i].point[0]={x=xmin+dx*i, y=ma*(xmin+dx*i)+ba,Bx=0,By=0,Ex=0,Ey=0,Vx=0,Vy=0,Ttot=0,S=Stepsize}--1mm if grid is in mm 
...
function Particle:move()--calculate the next postion of a particle
	for h=0, getn(self.par) do
		...
		--https://www.youtube.com/watch?v=jSziTlvmAeo
			...
			self.par[h].point[N+1]={Bx=0,By=0,Ex=0,Ey=0,x=0,y=0,Vx=0,Vy=0,Ttot=0,S=0} --create a new par[h].point
...
for x=0 , 12 do
	...
	simMa:Part(2,2,3,3,0,-3,10,"NdFeB 40 MGOe",90)--h1,h2,w1,w2,turn,POSX,POSY,MAT,volt_or_polari)
	simMa:Part(2,2,3,3,0,3,10,"NdFeB 40 MGOe",90)--h1,h2,w1,w2,turn,POSX,POSY,MAT,volt_or_polari)
	simMa:solve()
	...
	simEl:Part(10,1,1,0.01,0,0,18,"Nylon",-100)  --tip --h1,h2,w1,w2,turn,POSX,POSY,MAT,volt_or_polari)
	...
	par1:trace()
	simEl:solve()
	simEl:savePic(0,12,6,2,Voltage.."R")--(POSX,POSY,HIGHT,WIDTH,Infos)
	par1:DelTrace()
	
	par2=Particle:new(simEl,simMa,-0.1,16.9,0.1,16.9,21,0.0002)--ElecticalSim,MagneticalSim,xa1,ya1,xa2,ya2,Particlenumber)
	...
	simEl:savePic(0,12,6,2,Voltage)--(POSX,POSY,HIGHT,WIDTH,Infos)
